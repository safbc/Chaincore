angular.module('app.controllers', ['ionic', 'ionic.cloud', 'ngResource'])
  ...
  .controller('myAccountsCtrl',
    function ($scope, $state, $stateParams, $ionicAuth, $ionicUser, $timeout, $ionicLoading, $ionicModal,
      ...
      $scope.detail = function (acc) {

        $scope.acc = acc
        $scope.openModal();
      };

      $ionicModal.fromTemplateUrl('templates/accountInfo.html', {
        scope: $scope,
  ...
  .controller('newAccountCtrl',
    function ($scope, $state, $stateParams, $ionicAuth, $ionicUser, $ionicLoading, $ionicHistory,
      ...
      $scope.createAccount = function (form) {
        ...
        $scope.Request.account.keys = form.keys;
  ...
  .controller('newTradeCtrl',
    function ($scope, $state, $stateParams, $ionicAuth, $ionicUser, $ionicLoading, $ionicHistory,
      ...
      $scope.start = function () {
        // Get accountList
        $scope.getAccount($scope.userInfo.username);
        // first time get all assets.
        $scope.getAssets($scope.userInfo.username);
      ...
      $scope.setAction = function () {
        if ($scope.isSale) {
          ...
          $scope.assetList = $scope.accountsList[0].assets;
          $scope.form.asset = $scope.assetList[0].alias;
      ...
      $scope.getAccount = function (username) {
        ...
        $scope.svcAccounts.query(request).$promise
          .then(function (data) {
            if (data.length == 0) {
          ...
          .catch(function (data) {
            ...
            // });
            $scope.getBalances();
            console.log('Done: ' + JSON.stringify($scope.accountsList));
            // return data;
          });
      };
        ...
        function pr(requests) {
          ...
          Promise.all(requests)
            .then(function (items) {
              ...
              // Inject asset results values into $scope.accountList
              for (var index = 0; index < items.length; index++) {
                var _a = items[index];
                var _aa = _a[index];
                var _asset = {};

                _asset.alias = _aa.sumBy.assetAlias;
                _asset.id = _aa.sumBy.assetId;
                _asset.amount = _aa.amount;

                $scope.accountsList[index].assets.push(_asset);
              }
            })
            .catch(function (err) {
              console.log("err: " + err);
        ...
        $scope.svcAssets.query(request).$promise
          .then(function (data) {
            ...
            data.forEach(function (element) {
              ...
              _asset.alias = element.alias;
              _asset.id = element.id;
              _asset.amount = 0;
      ...
      /**
       * kick off the process to save the offer to the database.
       */
      $scope.makeOffer = function (formData) {

        var tradePost = {};
        tradePost.tradeAction = formData.tradeAction;
        tradePost.postedBy = formData.username;
        tradePost.expiryTimestamp = formData.expiryTimestamp;
        formData.asset = JSON.parse(formData.asset);
        // formData.account = JSON.parse(formData.account);

        tradePost.offerData = {
          "asset": {
            "id": formData.asset.id,
            "alias": formData.asset.alias // do I have both of these bits of information?
          },
          "account": {
            "id": formData.account.id,
            "alias": formData.account.alias
          },
          "amount": formData.amount
        };

        console.log(JSON.stringify(tradePost));

        // Send the data to the server
        $scope.svcTrades.save(tradePost).$promise
          .then(function (data) {
      ...
      /**
       * Calculate Offer expiry datetime
       */
      $scope.calcExpirydate = function (validDays) {
        ...
        $scope.form.expiryTimestamp = newdate;
      }
